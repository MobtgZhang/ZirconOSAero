...
3. **Scanout 路径**：将资源 **flush** 到 QEMU 可见 scanout，或与现有 `[src/drivers/video/core/framebuffer.zig](../../src/drivers/video/core/framebuffer.zig)` / **DWM** 合成输出对接。  
   **当前（x86_64）**：多段 `ATTACH_BACKING` + `SET_SCANOUT` + `RESOURCE_FLUSH`；`compositorTryRoundTripFramebufferRect`（≤32×32）仅诊断；光标 **vq1** `CMD_MOVE_CURSOR`；VirGL **feature bit0** 协商与 `CMD_CTX_CREATE`（`virgl=on`）；`SUBMIT_3D` / GPU 模糊仍占位。主合成与模糊仍 CPU。
