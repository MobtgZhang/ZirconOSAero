...
// Module: hal/aarch64/ramfb.zig
// Purpose: QEMU `virt` ramfb via fw_cfg when UEFI GOP is BLT-only or missing (common with virtio-gpu + EDK2).
//
...
/// QEMU ARM virt：VIRT_FW_CFG_BASE（见 QEMU `hw/arm/virt.h`）
pub const FW_CFG_BASE: usize = 0x09020000;
...
fn findRamfbKey() ?u16 {
    ...
    if (num_files > 64) {
        if (klog.DEBUG_MODE) klog.info("ramfb(a64): findRamfbKey num_files=%u > 64", .{num_files});
        return null;
    }
    var i: u32 = 0;
    ...
    if (klog.DEBUG_MODE) klog.info("ramfb(a64): no etc/ramfb in fw_cfg (add -device ramfb)", .{});
    return null;
}
...
/// 将 QEMU ramfb 扫描地址改到已有物理帧缓冲（例如日后 UEFI GOP 与 ramfb 并存时对齐扫描）
