...
| 物理伙伴封装 | `src/mm/phys_buddy.zig` | 自 `allocContiguous` carve arena，将伙伴索引映射到真实 PFN |
| Slab（小对象） | `src/mm/slab.zig` | 固定 `Obj` × `objects_per_slab`；与 `pool` 档位互补 |
| 物理帧位图 | `src/mm/frame.zig` | 全机物理页位图 + `alloc` / `allocContiguous` 线性扫描；审计见 [PHYS_ALLOC_AUDIT.md](PHYS_ALLOC_AUDIT.md) |
...
2. 在压力场景（大量小对象交错释放）下测量碎片；若 `heap_pos` 逼近 `HEAP_SIZE` 再引入 **TLSF** 或分离空闲链表（当前以 `heap` + `pool` + `slab` 为主）。  
3. 将热点小对象（句柄表节点等）迁移到专用 slab 档，减少通用堆压力。

## 测试

- `zig build test`：`heap`、`pool`、`buddy`、`slab` 主机用例；`heap.stats()` / `heap.heap_check()` 用于调试与压力测试收尾。
