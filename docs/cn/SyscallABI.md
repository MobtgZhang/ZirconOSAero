...
- **唯一用户态入口（与微软 x64 ntdll 一致）**：**`syscall` / `sysret`** — [`syscall_lstar.s`](../../src/arch/x86_64/syscall_lstar.s) + [`syscall_msr.zig`](../../src/arch/x86_64/syscall_msr.zig)（`IA32_LSTAR` / `IA32_STAR` / `IA32_FMASK`、`SWAPGS`、per-CPU RSP0）。用户态薄封装见 [`ntdll_syscall_win64.zig`](../../src/sdk/ntdll_syscall_win64.zig)（**勿**在 Linux 主机上执行其中的 `syscall`）。
- **启动**：在 [`main.zig`](../../src/main.zig) 中，于 `idt.init()` 之后、`enableInterrupts` 之前调用 `initSyscallInstructionPath()`。若 `zircon_x86_64_kernel_rsp0 == 0`，则跳过写 MSR（启动序异常，见 `syscall_msr.zig` 日志）。若 CPU **无** SYSCALL/SYSRET（CPUID 80000001H:EDX bit 11 为 0），则 **`keBugCheckEx`** 等价停机：已移除 IDT 向量 128 的 `int 0x80` 兼容路径，无 fast syscall 的 x86_64 目标 **不受支持**。成功使能时打印 `syscall/sysret: enabled`（见 `syscall_msr.zig`）。
- **IDT 向量 128**：与其它未用向量相同，指向默认桩；**不**再作为系统调用入口。
- **服务号**（`RAX`）：
  ...
  - **寄存器约定**：仅经 `syscall` 进入时使用上述 NT x64 约定（**不要**使用已移除的 `0x0010_0000` 内部服务号）。
