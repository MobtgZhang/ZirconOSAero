...
pub fn draw3DFrame(rect: Rect, raised: bool) void {
    ...
    if (theme.isGlassEnabled()) {
        const border_color = if (raised) colors.window_border else colors.button_shadow;
...
pub fn drawGlassSurface(rect: Rect) void {
    const colors = theme.getColors();
    if (colors.glass.enabled) {
        drawBlur(rect, colors.glass.blur_radius);
        fillRectAlpha(rect, colors.glass.colorization_color, colors.glass.colorization_alpha);
    } else {
        drawVGradient(rect, colors.titlebar_active_left, colors.titlebar_active_right);
    }
}
