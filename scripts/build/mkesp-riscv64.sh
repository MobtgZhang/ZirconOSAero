#!/usr/bin/env bash
# 构建 riscv64 UEFI ESP 镜像（GPT + FAT32）
# 包含 ZBM (BOOTRISCV64.EFI) 和内核 (kernel.elf)
#
# 用法: mkesp-riscv64.sh <out.img> <kernel.elf> <BOOTRISCV64.EFI>
# 环境变量:
...
if [ ! -f "${BOOT_EFI}" ]; then
    echo "[ZirconOS] ERROR: BOOTRISCV64.EFI 不存在: ${BOOT_EFI}" >&2
    exit 1
fi
...
if [ -f "$OUT" ] && command -v fuser >/dev/null 2>&1; then
    if fuser -v "$OUT" 2>&1 | grep -qi qemu; then
        echo "[ZirconOS] ERROR: ${OUT} 仍被 QEMU 占用，请先结束 qemu-system-riscv64。" >&2
        fuser -v "$OUT" 2>/dev/null || true
        exit 1
    fi
fi

echo "[ZirconOS] 构建 riscv64 ESP 镜像: ${OUT} (${DISK_MB}MB)"
...
# 复制 ZBM 引导程序
echo "[ZirconOS] ESP: 复制 BOOTRISCV64.EFI..."
run_mtools mcopy -p -n -i a: "$BOOT_EFI" a:/EFI/BOOT/BOOTRISCV64.EFI
echo "[ZirconOS] ESP: 已安装 BOOTRISCV64.EFI"
